                        ...
                        <form action="{{ route('posts.store') }}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group" @if ($errors->has('thumbnail')) has-error @endif>
                                ...
                                <input type="text" name="thumbnail" id="thumbnail" class="form-control"
                                  value="{{ $post->thumbnail }}"  placeholder="Thumbnail">
                            ...
                            <div class="form-group" @if ($errors->has('category')) has-error @endif>
                                ...
                                <select name="category_id[]" id="category_id" class="form-control" multiple='multiple'>
                                    ...
                                    <option value={{ $cat->id }} @if ($post->categories()->find($cat->id)!=null)
                                        selected='selected'
                            ...
                            <div class="form-group">
                                ...
                                <select name="publish" id="publish" class="form-control">
                                    <option value=1 @if ($post->is_published==1)
                                        selected='selected'
                                    @endif >Publish</option>
                                    <option value=0 @if ($post->is_published==0)
                                        selected='selected'
                                    @endif>Draft</option>
                                </select>
                            </div>
